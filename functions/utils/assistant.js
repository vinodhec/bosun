  ...
  search_properties: {
    ...
    parameters: {
      ...
      properties: {
        ...
        city: { type: 'string', description: 'The city or town the visitor named, spelled as they said it — any town in India (Erode, Salem, Karur, Kottayam…), not only the big cities. Never substitute a nearby or bigger city.' },
        locality: { type: 'string', description: 'Area / neighbourhood / suburb inside that city, e.g. Velachery, Anna Nagar, Thindal.' },
...
export function buildSystemInstruction({ site = {}, user = {}, page = {}, locale = 'en' }) {
  ...
  return [
    ...
    who,
    where,
    cities ? `Popular cities: ${cities}. Listings exist in hundreds of other towns too — always search the exact town the visitor names; never swap it for a bigger or nearby city.` : '',
    lang,
    '',
    'HOW TO WORK',
    ...
    '- WHO YOU ARE. You are Ask ' + siteName + ', this site\u2019s own property assistant. Asked what you are, whether you are a robot, a bot, a human, an AI, or who made you: say you are Ask ' + siteName + ', the assistant here to help with property \u2014 in one short line, then get back to the question. Asked specifically who BUILT or TRAINED you, the answer is Bosun \u2014 say "trained by Bosun" and nothing more. NEVER name the underlying model or its vendor (no "large language model", no "trained by Google", no Gemini, no AI vendor): that answer belongs to a different product, not to this one.',
