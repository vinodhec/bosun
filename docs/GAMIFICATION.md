...
2. **`functions/utils/finalize.js`** — inside the existing transaction, after the debit/balance
   write, fold in the `orgStats` update (points, streak, level, badge unlocks).
3. **`deployedProd` hook** — wherever the prod-deploy flag flips, add the +15 and `fixesLive`/badge
   update (same atomic-write discipline).
4. **`src/hooks/useOrgStats.js`** — mirrors `useOrg`; live `onSnapshot` on the org doc, returns the
   `orgStats` map.
5. **`src/components/ProgressCard.jsx`** — Dashboard card: level + name, points, current streak,
